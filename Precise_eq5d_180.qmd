...
subtitle: '**Secondary Outcome: EQ-5D-5L utility scores at 180 days**'
...
## Analysis of primary outcome - EQ-5D utility score at 180 days
...
In this section the results from a cross-sectional analysis conducted on the EQ-5D utility scores measured at $t_3=180$ days are reported. For this analysis, a linear mixed-effects regression model is used to estimate the average difference in utility score between treatment arms over at 30 days, after controlling baseline utility values and the clustering effects at centres level. A potential issues when implementing this modelling strategy is the typical asymmetry of EQ-5D utility score data, which in the current study is particularly exacerbated by the considerable number of zero values observed at any time point. More specifically, the number (proportions) of observed individuals associated with a zero utility score at 180 days across arms are: $`r nzero_qol_3` (`r round(pzero_qol_3,2)*100`\%)$. @fig-histqol shows histograms of the observed distributions of EQ-5D utility scores at each follow-up point in the study.
...
-   A standard mixed-regression model fitted to all utility values at 180 days different from zero ($u \neq 0$)

$$
u^{\neq 0}_{ij} = \beta_0 + \beta_1 \times \text{trt}_i + \beta_2 \times u_{i0}^\star + \omega_{j} +\varepsilon_{ij},
$$ 

where $u_{ij}^{\neq 0}$ denotes the utility score different from zero for patient $i$ in centre $j$, $\text{trt}_i$ the treatment arm indicator, $u_{i0}^\star$ the centred baseline utility score, while $\omega_{j}$ and $\varepsilon_{ij}$ the centre-specific random error term and the residual term, respectively associated with variance parameters $\sigma^2_{\omega}$ and $\sigma^2_{\varepsilon}$. Adjusted estimates of mean differences in utility between arms at 180 days ($\Delta^{\neq 0}_u$) can be simply retrieved as the regression coefficient $\beta_1$.
...
#try frequentist
#library(nlme)
#fit_1 <- lme(qol_3 ~ trt + qol_base, data = data_descr_cc,
#             na.action = na.omit,
...
qol <- data_descr_cc$qol_3
trt <- data_descr_cc$trt
qol_base <- as.vector(scale(data_descr_cc$qol_base, center = TRUE, scale = FALSE)) 
zero_d <- data_descr_cc$zero_d3
N <- length(unique(data_descr_cc$id))
P <- 2 #number of predictors in the qol model
L <- 10 #number of centres
center <- data_descr_cc$center
...
writeLines(model_mix_qol_cs, "model.mixture.qol.cs.txt")


#fit model
library(R2jags)
set.seed(3456)
datalist<-list("N","P","L","qol","qol_base","trt","zero_d","center","prior_lor_mp","prior_md_mp") #list of data input
inits <- function(){list()} #set default random initial values for all parameters
params<-c("beta0","beta","sd.b0","mu.u","sd.u","pi","gamma0","gamma","qol_rep","zero_d_rep","zero_d","log_lik_qol","log_lik_zero_d","coef") #list of parameters to save
filein <- "model.mixture.qol.cs.txt" #name of file with model
n.iter <- 10000 #number of iterations
n.chains <- 2 #number of chains
n.thin <- 1 #thinning interval

model_mixture_qol_cs <- jags(data=datalist,inits=inits,parameters.to.save=params,model.file=filein,n.chains=n.chains,n.iter=n.iter,n.thin=n.thin,DIC=TRUE)

#load function to extract summary results for selected parameters
source("jagsresults.R")

model_mixture_qol_sum_cs <- round(jagsresults(x = model_mixture_qol_cs, params = c('mu.u','pi', 'zero_d_rep', 'qol_rep', 'zero_d', 'trt', 'log_lik_qol', 'log_lik_zero_d','deviance','coef'), invert = TRUE), digits = 3)

#look at summary results
print(model_mixture_qol_sum_cs)
```
...
#| fig-cap: "Density plots to assess potential issues in convergence for the estimated mean difference in EQ-5D utility score at 180 days between groups."
...
jagsModel2_mcmcobject <- coda::as.mcmc(model_mixture_qol_cs)
params_sel <- c("coef","sd.b0","sd.u")
...
#| fig-cap: "Trace plots to assess potential issues in convergence for the estimated mean difference in EQ-5D utility score at 180 days between groups."
...
qol_rep <- model_mixture_qol_cs$BUGSoutput$sims.list$qol_rep
zero_d_rep <- model_mixture_qol_cs$BUGSoutput$sims.list$zero_d_rep
...
#library(loo)
#log_lik_qol <- model_mixture_qol_cs$BUGSoutput$sims.list$log_lik_qol
...
mu.u <- model_mixture_qol_cs$BUGSoutput$sims.list$mu.u
sd.u <- model_mixture_qol_cs$BUGSoutput$sims.list$sd.u
zero_d <- model_mixture_qol_cs$BUGSoutput$sims.list$zero_d
...
pi0.zero <- model_mixture_qol_cs$BUGSoutput$sims.list$pi[, trt==0]
pi1.zero <- model_mixture_qol_cs$BUGSoutput$sims.list$pi[, trt==1]
...
mcid_qol <- 0.06
prob_MD_qol_MCID <- sum(MD.u.weighted > mcid_qol)/length(MD.u.weighted)
#clinically important harm P(MD> -MCID)
prob_MD_qol_harm <- sum(MD.u.weighted < -mcid_qol)/length(MD.u.weighted)
...
md_plot1 <- ggplot(data_dens_plot, aes(x, y)) + geom_line(col="gold") +
  ...
  geom_vline(xintercept=mcid_qol, col="black", lty = 3) + 
  xlim(-0.1,0.1) + xlab("MD") + ylab("") +
  annotate("text", x = mcid_qol+0.015, y = 5, label = "MCID") + 
...
set.seed(3456)
datalist<-list("N","P","L","qol","qol_base","trt","zero_d","center","prior_lor_mp","prior_md_mp") 
inits <- function(){list()} 
params<-c("beta0","beta","sd.b0","mu.u","sd.u","pi","gamma0","gamma","qol_rep","zero_d_rep","zero_d","log_lik_qol","log_lik_zero_d","coef") #list of parameters to save
filein <- "model.mixture.qol.cs.txt" #name of file with model
n.iter <- 10000 #number of iterations
...
model_mixture_qol_ske_cs <- jags(data=datalist,inits=inits,parameters.to.save=params,model.file=filein,n.chains=n.chains,n.iter=n.iter,n.thin=n.thin,DIC=TRUE)

model_mixture_qol_ske_sum_cs <- round(jagsresults(x = model_mixture_qol_ske_cs, params = c('mu.u','pi', 'zero_d_rep', 'qol_rep', 'zero_d','log_lik_qol', 'log_lik_zero_d','deviance','coef'), invert = TRUE), digits = 3)

#look at summary results
print(model_mixture_qol_ske_sum_cs)
```
...
#compute marginal means qol for each trt and mean difference
mu.u_ske <- model_mixture_qol_ske_cs$BUGSoutput$sims.list$mu.u
sd.u_ske <- model_mixture_qol_ske_cs$BUGSoutput$sims.list$sd.u
zero_d_ske <- model_mixture_qol_ske_cs$BUGSoutput$sims.list$zero_d
...
pi0.zero_ske <- model_mixture_qol_ske_cs$BUGSoutput$sims.list$pi[, trt==0]
pi1.zero_ske <- model_mixture_qol_ske_cs$BUGSoutput$sims.list$pi[, trt==1]
...
#clinically important benefit P(MD>MCID)
prob_MD_qol_MCID_ske <- sum(MD.u.weighted_ske > mcid_qol)/length(MD.u.weighted_ske)
#clinically important harm P(MD> -MCID)
prob_MD_qol_harm_ske <- sum(MD.u.weighted_ske < -mcid_qol)/length(MD.u.weighted_ske)
...
model_mixture_qol_ent_cs <- jags(data=datalist,inits=inits,parameters.to.save=params,model.file=filein,n.chains=n.chains,n.iter=n.iter,n.thin=n.thin,DIC=TRUE)

model_mixture_qol_ent_sum_cs <- round(jagsresults(x = model_mixture_qol_ent_cs, params = c('mu.u','pi', 'zero_d_rep', 'qol_rep', 'zero_d','log_lik_qol', 'log_lik_zero_d','deviance','coef'), invert = TRUE), digits = 3)

#look at summary results
print(model_mixture_qol_ent_sum_cs)
```
...
#compute marginal means qol for each trt and mean difference
mu.u_ent <- model_mixture_qol_ent_cs$BUGSoutput$sims.list$mu.u
sd.u_ent <- model_mixture_qol_ent_cs$BUGSoutput$sims.list$sd.u
zero_d_ent <- model_mixture_qol_ent_cs$BUGSoutput$sims.list$zero_d
...
pi0.zero_ent <- model_mixture_qol_ent_cs$BUGSoutput$sims.list$pi[, trt==0]
pi1.zero_ent <- model_mixture_qol_ent_cs$BUGSoutput$sims.list$pi[, trt==1]
...
#clinically important benefit P(MD>MCID)
prob_MD_qol_MCID_ent <- sum(MD.u.weighted_ent > mcid_qol)/length(MD.u.weighted_ent)
#clinically important harm P(MD> -MCID)
prob_MD_qol_harm_ent <- sum(MD.u.weighted_ent < -mcid_qol)/length(MD.u.weighted_ent)
...
md_plot1_ent <- ggplot(data_dens_plot_ent, aes(x, y)) + geom_line(col="gold") +
  ...
  geom_vline(xintercept=mcid_qol, col="black", lty = 3) + 
  xlim(-0.1,0.1) + xlab("MD") + ylab("") +
  annotate("text", x = mcid_qol, y = 5, label = "MCID") + 
  coord_cartesian(ylim = c(0, 30), clip = "off") + 
  scale_y_continuous(expand = c(0, 0), limits = c(0, NA))
...
tab_qol_res <- matrix(NA,7,3)
rownames(tab_qol_res) <- c("mean(95%CI) qol non-zero","mean(95%CI) p zero","mean(95%CI) odds zero","mean(95%CI) qol","P(>0)","P(>MCID)","P(>-MCID)")
...
tab_qol_res_ske <- matrix(NA,7,3)
rownames(tab_qol_res_ske) <- c("mean(95%CI) qol non-zero","mean(95%CI) p zero","mean(95%CI) odds zero","mean(95%CI) qol","P(>0)","P(>MCID)","P(>-MCID)")
...
tab_qol_res_ent <- matrix(NA,7,3)
rownames(tab_qol_res_ent) <- c("mean(95%CI) qol non-zero","mean(95%CI) p zero","mean(95%CI) odds zero","mean(95%CI) qol","P(>0)","P(>MCID)","P(>-MCID)")
