...
subtitle: '**Secondary Outcome: EQ-5D-5L utility scores at 90 days**'
...
## Analysis of primary outcome - EQ-5D utility score at 90 days
...
In this section the results from a cross-sectional analysis conducted on the EQ-5D utility scores measured at $t_2=90$ days are reported. For this analysis, a linear mixed-effects regression model is used to estimate the average difference in utility score between treatment arms over at 30 days, after controlling baseline utility values and the clustering effects at centres level. A potential issues when implementing this modelling strategy is the typical asymmetry of EQ-5D utility score data, which in the current study is particularly exacerbated by the considerable number of zero values observed at any time point. More specifically, the number (proportions) of observed individuals associated with a zero utility score at 90 days across arms are: $`r nzero_qol_2` (`r round(pzero_qol_2,2)*100`\%)$. @fig-histqol shows histograms of the observed distributions of EQ-5D utility scores at each follow-up point in the study.
...
#try frequentist
#library(nlme)
#fit_1 <- lme(qol_2 ~ trt + qol_base, data = data_descr_cc,
#             na.action = na.omit,
...
#| fig-cap: "Density plots to assess potential issues in convergence for the estimated mean difference in EQ-5D utility score at 90 days between groups."
...
#| fig-cap: "Trace plots to assess potential issues in convergence for the estimated mean difference in EQ-5D utility score at 90 days between groups."
